...
def main():
  ...
  with open(input_file_name, "r") as f:
    ...
    for i in range(2, len(lines)):
      ...
      for j in range(len(items)):
        ...
        if types[j] == "float":
          ...
          feature_dict[keys[j]] = _bytes_feature(value=codecs.decode(item, "utf-8").encode("utf8"))
    ...
    serialized_proto = tf.contrib.util.make_tensor_proto(serialized_strings, dtype=tf.string)
    serialized_proto_handle = serialized_proto.string_val
    print(serialized_proto_handle)
